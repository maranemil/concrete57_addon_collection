...
<link href="<?php echo "/packages/html_plus/blocks/html_plus/styles/"; ?>/codemirror.css" rel="stylesheet"
      type="text/css">
...
<script>
    ...
    (function () {
        ...
        $.getScript("/packages/html_plus/blocks/html_plus/scripts/codemirror.js", function () {
            ...
            $(".html-element").click(function (e) {
                ...
                e.preventDefault();
                const elem = this;
                const highlighted = htmlEditor.getSelection();
                const data = elem.getAttribute('data-tag');

                //If something is highlighted wrap in the new tags where possible
                if (htmlEditor.somethingSelected()) {
                    const newData = data.replace("><", ">" + highlighted + "<");
    ...
    function resizeEditor() {
        let optsHeight = $("#htmlEditor-opts").height();
        const modalHeight = $(".ui-resizable").height();

        // 130px allows for the modal header and footer
        const containerHeight = (modalHeight - optsHeight - 130) + "px";
    ...
    function insertLink(pageId, pageName) {
        ...
        let highlighted = htmlEditor.getSelection();
        if (htmlEditor.somethingSelected()) {
...
<div id="htmlEditor-container">
    ...
    <label for="htmlPlusContent"></label><textarea id="htmlPlusContent" name="htmlPlusContent" style="display:none;">
        <?php
