...
/**
 * @property $tumblrUsername
 * @property $postsPerPage
 * @property $hideTitles
 * @property $dateFormat
 * @property $charsPerEntry
 * @property $preferedImageWidth
 * @property $entryURL
...
class Controller extends BlockController
{
    ...
    public function insertPagination($postsPerPage, $nPostsInFeed, $currentPage)
    {
        ...
        $url = (new BlockViewExpand)->actionOpen("getBlogHTML"); //
        ...
        if ($currentPage > 1) {
            echo '<a href="javascript:getBlog(\'' . $url . '\',' . ($currentPage - 1) . ',\'' . $this->tumblrUsername . '\')">Prev</a>&nbsp;&nbsp;';
        }
        for ($i = $lowPage; $i <= $highPage; ++$i) {
            ...
            if ($i !== $currentPage) {
                echo '<a href="javascript:getBlog(\'' . $url . '\',' . $i . ',\'' . $this->tumblrUsername . '\')">[' . $i . ']</a>';
            } else {
                echo '[' . $i . ']';
            }
        }
        if ($currentPage < $nPagesInFeed) {
            echo '&nbsp;&nbsp;<a href="javascript:getBlog(\'' . $url . '\',' . ($currentPage + 1) . ',\'' . $this->tumblrUsername . '\')">Next</a>';
        }
        echo '&nbsp;&nbsp;<a href="javascript:getBlog(\'' . $url . '\',' . $nPagesInFeed . ',\'' . $this->tumblrUsername . '\')">Oldest</a>';
    }

    public function parseHtmlNumChars($htmlToParse, $charLimit)
    {
        $chunks = preg_split('/<.*?>/', $htmlToParse);
        $numChars = 0;
        $lastChunk = '';
        foreach ($chunks as $thisChunk) {
            if (preg_match('/^\s*$/', $thisChunk)) {
                continue;
            }
    ...
    public function action_getBlogHTML()
    {
        ...
        while (!$contents) {
            ...
            $contents = Loader::helper('file')->getContents('https://' . $this->tumblrUsername . '.tumblr.com/api/read/json?start=' . $tumblrFeedStart . '&num=' . $this->postsPerPage);
            if (!strpos($contents, 'tumblr_api_read')) {
                $contents = false;
            }
        }
        ...
        foreach ($tumblr->{'posts'} as $post) {
            if ($notFirst) {
                $borderClass = ' tumblrfeed-topborder';
            } else {
                $borderClass = '';
            }
            $notFirst = true;
            ...
            switch ($post->{'type'}) {
                ...
                case 'photo':
                    ...
                    if ($this->preferedImageWidth > 500 & $post->{'photo-url-1280'} != '') {
                        $photoURL = $post->{'photo-url-1280'};
                    } elseif ($this->preferedImageWidth > 400 & $post->{'photo-url-500'} != '') {
                        $photoURL = $post->{'photo-url-500'};
                    } elseif ($this->preferedImageWidth > 250 & $post->{'photo-url-400'} != '') {
                        $photoURL = $post->{'photo-url-400'};
                    } elseif ($this->preferedImageWidth > 100 & $post->{'photo-url-250'} != '') {
                        $photoURL = $post->{'photo-url-250'};
                    } elseif ($this->preferedImageWidth > 75 & $post->{'photo-url-100'} != '') {
